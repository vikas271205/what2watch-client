    ...
    const fetchHollywoodMovies = async () => {
      try {
        ...
        if (!Array.isArray(data)) {
            console.error("Hollywood API did not return an array:", data);
            setMovies([]);
            return;
        }
          ...
          filtered.map(async (movie) => {
            ...
            return {
              ...
              genres: movie.genre_ids.map(id => {
                // Simplified genre mapping since we don't fetch the map here
                // For a full solution, a global genre context would be best
  ...
  return (
    <div className="relative group">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/hollywood" className="text-xs font-semibold bg-gray-100 text-gray-700 px-4 py-2 rounded-full hover:bg-gray-200 transition-colors dark:bg-white/10 dark:text-white dark:hover:bg-white/20">
          See All
        </Link>
      </div>
      ...
      {isSectionLoading ? (
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm">No Hollywood movies found.</p>
      )}
    </div>
  );
}
