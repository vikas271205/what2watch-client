...
function MovieCard({
  ...
  const toggleSave = async (e) => {
    ...
    try {
      ...
      if (isSaved) {
        await deleteDoc(ref); setIsSaved(false);
      } else {
        await setDoc(ref, {
          userId: user.uid, [isTV ? "tvId" : "movieId"]: isTV ? `${id}_tv` : id,
          title, imageUrl, rating: tmdbRating, timestamp: serverTimestamp(),
        });
        setIsSaved(true);
      }
  ...
  const handleAdminDelete = async (e) => {
    ...
    if (result.success) { onDelete(`${type}_${id}`); } 
