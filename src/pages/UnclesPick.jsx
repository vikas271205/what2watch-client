  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-gray-900 to-black text-white pt-20">
      ...
      <div className="p-4 sm:p-6 max-w-7xl mx-auto relative z-10">
        <motion.h1
          className="text-4xl sm:text-5xl font-bold mb-6 text-center bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600"
        ...
        >
          ...
          <motion.input
            ...
            className="px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300 hover:bg-gray-700 w-28"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          />
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="h-[280px] bg-gray-800 rounded-lg animate-pulse" />
            ))}
          </div>
        ) : (
          sections.map((section) => (
            <div key={section.title} className="mb-12">
              <motion.h2
                className="text-2xl sm:text-3xl font-semibold mb-4 text-purple-300"
                          ...
                          isAdmin
                            ? async () => {
                                ...
                                if (result.success) {
                                  ...
                                  setItems((prev) =>
                                    prev.filter((i) => `${i.type}_${i.id}` !== `${item.type}_${i.id}`)
                                  );
                                } else {
